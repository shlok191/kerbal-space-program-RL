...
class KSPEnvironment:
    ...
    def __init__(
        ...
        self.vessel_stream = self.connection.add_stream(
            getattr, self.connection.space_center, "active_vessel"
        )
    ...
    def get_vessel_updates(self) -> Dict[str, float]:
        ...
        vessel_state = {
            ...
            "fuel": vessel.resources.amount(
                "LiquidFuel"
            ),  # Liquid fuel remaining [units]
            "max_fuel": vessel.resources.max(
                "LiquidFuel"
            ),  # Max liquid fuel capacity [units]
            "oxidizer": vessel.resources.amount(
                "Oxidizer"
            ),  # Oxidizer remaining [units]
            "max_oxidizer": vessel.resources.max(
                "Oxidizer"
            ),  # Max oxidizer capacity [units]
            "electric_charge": vessel.resources.amount(
                "ElectricCharge"
            ...
            vessel_state.update(
                {
                    ...
                    "monopropellant": self.vessel.resources.amount(
                        "MonoPropellant"
                    ),  # Remaining monopropellant [units]
                    "monopropellant_capacity": self.vessel.resources.max(
                        "MonoPropellant"
                    ),  # Max monopropellant capacity [units]
                }
            )
    ...
    def update_vehicle_controls(self, controls: Dict[str, float]):
        ...
        self.vessel.control.throttle = clamp(
            controls.get("throttle", self.vessel.control.throttle)
        )
        self.vessel.control.pitch = clamp(
            controls.get("pitch", self.vessel.control.pitch)
        )
        ...
        if self.rcs_enabled:
            ...
            self.vessel.control.up = clamp(
                controls.get("rcs_up", self.vessel.control.up)
            )
            self.vessel.control.down = clamp(
                controls.get("rcs_down", self.vessel.control.down)
            )
            self.vessel.control.left = clamp(
                controls.get("rcs_left", self.vessel.control.left)
            )
            self.vessel.control.right = clamp(
                controls.get("rcs_right", self.vessel.control.right)
            )
            self.vessel.control.forward = clamp(
                controls.get("rcs_forward", self.vessel.control.forward)
            )
            self.vessel.control.backward = clamp(
                controls.get("rcs_backward", self.vessel.control.backward)
            )
    ...
    def get_distances(self, target_name: str) -> Dict[str, float]:
        ...
        distances = {
            ...
            "distance_to_target": self.vessel.orbit.body.surface_position(
