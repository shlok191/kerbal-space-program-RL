...
def test_ksp_connection(
    ...
    try:
        ...
        from kerbal_rl.environments.orbital import KerbinOrbitalEnvironment
...
def _log_raw_vessel_state(raw_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logging.getLogger(__name__).info(
        f"Available Thrust: {raw_state['avail_thrust']:.2f} N"
    )
    ...
    logging.getLogger(__name__).info(
        f"Electric Charge: {raw_state['electric_charge']:.2f} units"
    )
