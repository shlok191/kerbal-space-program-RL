from ..environments.orbital import KerbinOrbitalEnvironment, MissionStatus
from utils import ActorNetwork, CriticNetwork, GAE_function
...
def collect_experience(
    ...
    for _ in range(steps):
        ...
        curr_vehicle_state = env.get_normalized_vessel_state()
        action = actor(curr_vehicle_state)
...
def train_ppo(
    ...
    for iteration in range(total_steps // steps_per_batch):
    
        observations, actions, rewards, dones = collect_experience(
            env=environment,
            actor=actor_network.apply(training_state.actor_params),
            steps=steps_per_batch
        )
        
        values = critic_network.apply(training_state.critic_params, observations)
